...
function ui() {
  ...
  valIn.addEventListener("input", () => {
    const norm = normalizeDigits(valIn.value)
    if (norm !== valIn.value) valIn.value = norm
  })
  regIn.addEventListener("input", () => {
    const norm = normalizeDigits(regIn.value)
    if (norm !== regIn.value) regIn.value = norm
  })
